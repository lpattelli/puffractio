...
class FHD:        
    ##Define Gabor filter miming the gaborimfilt of matlab
    def gabor(image, wavelength, orientation):
        wavelength = wavelength
        orientation = orientation / 180 * math.pi    # in radian, and seems to run in opposite direction
        ...
        gabor_int = cv2.filter2D(image, -1, gabor_filter_real) + 1j * cv2.filter2D(image, -1, gabor_filter_imag)
        mag = np.abs(gabor_int)
        phase = np.angle(gabor_int)
        return mag, phase
